...
def scheme_load_all(directory, env):
   """
   Loads all .scm files in the given directory, alphabetically. Used only in tests/ code.
   """
...
def run(*argv):
   ...
   read_eval_print_loop(next_line, create_global_frame(), startup=len(load_files)>0, interactive=interactive, load_files=load_files)
   tscheme_exitonclick()